...
def check_tar_archive(tar_obj, md_checklist=None):
    ...
    logger.info('Getting checksums of files in archive {}'.format(tar_obj))
    md_list = []
    verified = None if md_checklist is None else True
    for member in tar_obj:
        with closing(tar_obj.extractfile(member)) as md_file:
            md_hash = md5(md_file)
            md_list.append([member.name, md_hash])
            logger.debug('{}, md5:{}'.format(member.name, md_hash))
...
def compress_folder(source_fold, dest_path, dest_name, mode='w:'):
    ...
    :param dest_name: string, destination filename base
    :param mode: string, compression mode to pass tarfile ('w:' no compression, 'w:xz' for lhz, se tarfile documentation for more)
    :return: (string, string, list), full path of created file, md5 of the file, list of md5 of archived files
    """
    ext = mode.split(':')[-1]
    fold_orig = os.path.split(source_fold)[0]
    file_name = '{0}.tar'.format(dest_name)
    if not ext == '':
        file_name += '.{0}'.format(ext)
    dest_file = os.path.join(dest_path, file_name)
    dest_md5_list = os.path.join(dest_path, '{0}.{1}'.format(dest_name, 'mdl'))
    dest_md5 = os.path.join(dest_path, '{0}.{1}'.format(dest_name, 'md5'))
    logger.info('Will compress folder {0} from {1} into {2}'.format(dest_name, fold_orig, dest_file))
